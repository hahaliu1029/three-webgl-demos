/**
 * 使用Three.js和Cannon-es物理引擎的3D物理球与平面交互示例。
 * 
 * 功能概述:
 * - 初始化3D场景、相机和渲染器。
 * - 创建3D球体和平面并加入到场景中。
 * - 初始化Cannon-es物理世界，并为球体和平面添加物理属性。
 * - 当球体与平面发生碰撞时，播放击打声音。
 * - 添加环境光和平行光以实现真实感的光照效果。
 * - 使用OrbitControls实现场景旋转。
 * - 在动画循环中更新Cannon-es物理世界，并将物理属性同步到3D物体上。
 * 
...
// 创建球体材质
const sphereMaterial = new CANNON.Material('default');
